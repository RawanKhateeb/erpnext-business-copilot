...
class AIReportGenerator:
    ...
    def _build_prompt(summary: Dict[str, Any], query: str = None) -> str:
        ...
        if top_suppliers:
            ...
            for i, (name, amount) in enumerate(top_suppliers[:5], 1):
                suppliers_text += f"\n{i}. {name}: ${amount:,.2f}"
        ...
        prompt = f"""Generate a professional procurement report for {date_range} based on this data:
...
Please create a concise narrative report that:
1. Summarizes overall spending and order activity
2. Highlights key trends and patterns
3. Identifies top suppliers and their contribution
4. Notes any concerns (e.g., high pending orders)
5. Provides one actionable recommendation

Keep the report to 3-4 paragraphs. Use professional business language."""
